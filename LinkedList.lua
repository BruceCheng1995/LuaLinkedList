...
function LinkedList:AddAfter(node, value)
	if not self:ValidateNewNode(node) then return end
	local newnode = LinkedNode:new(value, self)
...
function LinkedList:AddBefore(node, value)
	if not self:ValidateNode(node) then return end
	local newnode = LinkedNode:new(value, self)
...
function LinkedList:AddNodeAfter(node, newnode)
	if not self:ValidateNode(node) and not self:ValidateNewNode(newnode) then return end
	InternalInsertNodeBefore(self, node.Next, newnode)
	newnode.List = self
end
...
function LinkedList:AddNodeBefore(node, newnode)
	...
	InternalInsertNodeBefore(self, node, newnode)
	newnode.List = self
	if node ~= self.First then return end
	self.First = newnode
end
--Remove
...
function LinkedList:CopyTo(tab, index)
	...
	local ptrnode = self.First
	if ptrnode == nil then return end
	repeat
...
return {	list = setmetatable(LinkedList, {__call = LinkedList.new}),
